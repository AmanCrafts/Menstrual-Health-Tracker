...
const generatePeriodLogs = (userId, cycleLength, periodLength, profile) => {
    ...
    for (let i = 0; i < numCycles; i++) {
        ...
        if (symptomSeverity === 'severe') {
            ...
            symptoms = randomFrom([['cramps'], ['fatigue'], ['bloating'], []]);
        }
...
const getProfileNotes = (profile, _cycleIndex) => {
    const notes = {
        regular: [
        ...
        short: ['Quick and light as usual', 'Barely any symptoms', 'Easy cycle', '', ''],
        heavy: [
            'Very heavy flow',
...
const generateMoodLogs = (userId, periodLogs, profile) => {
    ...
    periodLogs.forEach((period, index) => {
        ...
        logs.push({
            ...
            intensity:
                profile === 'heavy' || profile === 'irregular' ? randomInt(3, 5) : randomInt(2, 4),
...
const getSymptomNotes = (profile, phase) => {
    if (phase === 'pms') {
        ...
        const notes = {
            ...
            heavy: [
                'Severe cramps, staying home',
                'Pain is unbearable',
                'Heating pad and medication needed',
                'Considering doctor visit',
            ],
...
async function seedDatabase() {
    try {
        ...
        console.log(
            '📍 Database:',
            config.mongodb.uri.includes('mongodb.net') ? 'MongoDB Atlas' : 'Local MongoDB'
        );
