    ...
    const mostCommon = Object.entries(symptomCounts)
      ...
      .map(([symptom, count]) => {
        ...
        const avgIntensity = intensities.length > 0
          ? intensities.reduce((sum, i) => sum + (intensityMap[i] || 2), 0) / intensities.length
          : 2;
    ...
    if (painLogs.length > 0) {
      ...
      if (highPainDays > 3) {
        insights.push({
          ...
          recommendation: 'Consider discussing pain management options with a healthcare provider.',
    ...
    res.status(200).json({
      ...
      data: {
        ...
        summary: {
          ...
          averageSymptomPerDay: logs.length > 0 
            ? (Object.values(symptomCounts).reduce((a, b) => a + b, 0) / logs.length).toFixed(1)
            : 0
        }
      }
    });
