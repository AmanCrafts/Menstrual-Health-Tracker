...
export const getCycleStats = async (req, res) => {
    try {
        ...
        if (
            stats.regularity.category === 'very_regular' ||
            stats.regularity.category === 'regular'
        ) {
            insights.push({
                type: 'positive',
            ...
            insights.push({
                ...
                description:
                    'Your cycle length varies significantly. Factors like stress, weight, or hormonal changes may be involved.',
        ...
        if (stats.averageCycleLength < 21) {
            insights.push({
                ...
                title: 'Short Cycles',
                description:
                    'Your average cycle is less than 21 days, which may warrant attention.',
            ...
            insights.push({
                ...
                title: 'Long Cycles',
                description:
                    'Your average cycle is over 35 days. This can be normal but worth monitoring.',
                icon: 'clock',
            });
        }
...
export const getCyclePredictions = async (req, res) => {
    try {
        ...
        const stats = await PeriodLog.getCycleStatistics(req.user._id);
        const { averageCycleLength, averagePeriodLength, regularity, trend, lastPeriod } = stats;
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const monthsAhead = Math.floor((nextPeriod - today) / (30 * 24 * 60 * 60 * 1000));
        ...
        res.status(200).json({
            ...
            data: {
                ...
                currentCycleDay,
                daysUntilNextPeriod:
                    daysUntilNextPeriod > 0 ? daysUntilNextPeriod : averageCycleLength,
                currentPhase,
                regularity: {
                ...
                confidence: {
                    ...
                    level:
                        baseConfidence >= 80 ? 'high' : baseConfidence >= 60 ? 'moderate' : 'low',
                    factors: [
